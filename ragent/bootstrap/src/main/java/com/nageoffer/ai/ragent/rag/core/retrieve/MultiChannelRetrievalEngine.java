...
public class MultiChannelRetrievalEngine {
    ...
    private List<SearchChannelResult> executeSearchChannels(SearchContext context) {
        // Step 1: 动态选择启用的通道
        /*
        * 1️⃣ 意图定向检索通道（优先级=1，最高）
            启用条件（需同时满足）：
            ✅ 配置开关开启：rag.search.channels.intent-directed.enabled=true
            ✅ 存在 KB 类型意图节点且分数 ≥ 0.4
            2️⃣ 向量全局检索通道（优先级=10，较低）
            启用条件（满足任一即可）：
            ✅ 配置开关开启：rag.search.channels.vector-global.enabled=true
            ✅ 条件A：未识别出任何意图
            ✅ 条件B：最高意图置信度 < 0.6（阈值可配置）
        * */
        ...
        // Step 2: 并行执行所有通道 并行执行所有通道
        List<CompletableFuture<SearchChannelResult>> futures = enabledChannels.stream()
                .map(channel -> CompletableFuture.supplyAsync(
                        () -> {
                            try {
                                log.info("执行检索通道：{}", channel.getName());
                                //每个通道独立执行 channel.search(context)
